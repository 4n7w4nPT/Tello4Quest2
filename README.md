...
**v0.4** — a landscape-layout Menu screen with a five-item pre-flight checklist, the left/right telemetry bands reworked (live time-series graphs on the left, a wider mini-map + activity log on the right, both angled toward the pilot like a cockpit console), and manual image controls (white balance, brightness, contrast) alongside the automatic night mode/sharpening from v0.3. See [Known limitations](#known-limitations--ideas-for-improvement) for what's still rough around the edges, and [Building the project](#building-the-project) for a setup requirement that determines whether the app launches at all on some headset configurations.

<details>
<summary>v0.3 — real .mp4 recording, mini-map, automatic video enhancement</summary>

This release added real `.mp4` recording (via Android's MediaMuxer, no more raw `.h264` needing ffmpeg to become watchable), a flight-path mini-map, and automatic video enhancement (night mode + sharpening).
</details>
...
- **Three-screen flow**: a Menu screen (landscape, pre-flight checklist + button legend), the Piloting screen (video + telemetry, gamepad live), and a Settings screen — see [Controls](#controls) below for the exact button mapping on each.
- Automatic connection to the Tello over WiFi direct, no manual setup, with automatic reconnection if the link drops.
- Live H.264 video decoded and displayed on a floating screen, world-locked (it does **not** follow your head). Distance, size, and vertical position are adjustable in Settings, alongside a full set of image controls: automatic night mode (per-pixel brightness lift + adaptive noise blur on dark footage, self-limiting so well-lit footage is untouched) and sharpening (both on by default, strength adjustable), plus manual white balance, brightness, and contrast sliders (all neutral/off by default) for correcting a color cast or exposure that's specific to your lighting - a small camera sensor's automatic white balance can skew noticeably warm indoors, and no automatic correction is applied for that since it risks overcorrecting a scene that's genuinely warm-toned.
...
- Live telemetry: top banner (gamepad/Tello/video/last-command status + temperature), bottom banner (altitude, flight time, ground speed, estimated time remaining, batteries), and two side bands - both sized to match the video screen's height and angled ~20° toward the pilot like a cockpit side console (mirrored angles, so they face each other symmetrically):
  - **Left**: four live time-series graphs (video FPS, battery %, altitude, temperature), each on its own fixed or Settings-driven axis. The time axis starts at a 2-minute window and smoothly widens (never scrolls) once a flight runs longer, so the whole flight's history stays visible rather than losing the start. History persists for the whole app session, not reset per-flight.
  - **Right**: a flight-path **mini-map** on top (75% of the band) - north-up (never rotates, only the drone's heading icon does), a persistent trail of connected line segments (not just scattered dots), and a zoom level based on the largest distance ever reached from the takeoff point this flight (never shrinks mid-flight, so the scale doesn't jitter) - and an **activity log** below it (25%) - a running, first-person transcript of what the drone is doing and noticing ("Alright, taking off." / "Getting warm up here, keep an eye on me." / "Something's pushing me around — might be wind."), oldest entry at the top fading toward the newest at the bottom.
...
- A Settings screen (reached from the Menu) with ~35 adjustable parameters — screen placement/size, image controls, flight safety thresholds, gamepad feel, panel placement — grouped by section, gamepad-navigable, with a one-press "reset to defaults" and everything persisted across app restarts.
- A pre-flight gate screen that waits for five checks (Bluetooth enabled, gamepad connected, Wi-Fi enabled, Tello Wi-Fi connected, video feed connected) before allowing takeoff, all kept live in the background even while flying, so returning to the menu never shows stale status.
...
| North | Open Settings |
| East | Quit the app |
| West | Currently inactive — see [Known limitations](#known-limitations--ideas-for-improvement) |
...
Photos, videos, and flight logs are all written via Android's **MediaStore** API into the shared collections (`Pictures/`, `Movies/`, `Download/`), each under a `Tello4Quest2` subfolder — not the app's private storage. Inserting your own new items into MediaStore doesn't require any special manifest permission on modern Android (scoped storage), and it means everything the app produces is immediately visible from the headset's own Files app, from MQDH's File Manager on a connected PC, and over a plain USB file transfer — the same way Quest's own screenshots and recordings are.
...
- **Meta XR SDK / OVRPlugin** for passthrough and headset tracking, including the Tracking Origin Type / Guardian boundary interaction documented under [Building the project](#building-the-project) - not something we'd have gotten right by guessing.
...
| `TelloVideoDecoder.cs` | Hardware decoding via PopH264; handles both possible output formats (direct RGBA/BGRA, or 2-plane YUV NV12, depending on the device); waits for a live SPS before feeding the decoder, and captures the real SPS/PPS bytes from the stream (never hardcoded) for `TelloVideoRecorder` to reuse. |
| `TelloVideoDisplay.cs` | Displays the feed on a world-locked quad; handles zoom/continuous size, opacity, and the manual white balance/brightness/contrast controls. |
| `TelloYuvNV12ToRGB.shader` + `TelloVideoYUV.mat` / `TelloVideoRGBA.mat` | GPU-side YUV→RGB conversion (BT.601) with automatic night mode, sharpening, and the manual white balance/brightness/contrast adjustments; and the direct-RGBA material. |
| `TelloVideoRecorder.cs` | Records the video feed as a real `.mp4` via Android's MediaMuxer (zero re-encoding), using the SPS/PPS `TelloVideoDecoder` captured live from the stream. |
...
| `TelloInitGate.cs` | Owns the Menu/Piloting/Settings screen state machine, the landscape pre-flight checklist (five checks, two of them read Android's Bluetooth/Wi-Fi state directly), the X-cross button legend (brand-aware prompts, optional icon glyphs), and hand-off to the flight display. |
| `TelloWifiConnector.java` | A small compiled Java helper for connecting to the Tello's own Wi-Fi hotspot by SSID prefix. Built and functional, but currently **not wired to any input** (see Known limitations) - kept in the repo since the West button's legend card still references it and it may come back. |
| `TelloSettingsScreen.cs` | The scrollable Settings screen — ~35 parameters across Display/Safety/Gamepad/Panels sections, gamepad navigation, reset-to-defaults, persistence. |
| `TelloStatusPanel.cs` / `TelloOptionsPanel.cs` | Telemetry banners above/below the video screen. |
| `TelloSpatialPanel.cs` | Left-side band: four live time-series graphs (FPS, battery, altitude, temperature), 2x2, angled toward the pilot. |
| `TelloActionLogPanel.cs` | Right-side band: a flight-path mini-map (top 75%) and the drone's first-person activity log (bottom 25%), angled toward the pilot (mirrored from the left band). |
| `TelloUiKit.cs` | Shared UI utilities — procedural rounded-sprite/arrow-icon generation, card-shell building, fixed camera-relative placement math, and gamepad brand/button-prompt detection. |
...
3. Copy `TelloWifiConnector.java` to `Assets/Plugins/Android/src/main/java/com/tello4quest2/TelloWifiConnector.java` (create the folder structure if it doesn't exist). Unity compiles any `.java` source found there automatically as part of the Gradle build - no separate AAR project needed. This is currently unused (see Known limitations) but still needs to be present for the project to compile, since `TelloInitGate.cs` references it.
4. Add the two Wi-Fi permissions to your manifest: if you don't already have a custom one, copy `AndroidManifest.xml` from this repo to `Assets/Plugins/Android/AndroidManifest.xml`. If you already have one, just add its two `<uses-permission>` lines to yours instead of overwriting the file. Unity's Gradle build merges this additively with its own auto-generated manifest.
...
| `TelloSpatialPanel` | `TelloSpatialPanel` | Reference `TelloConnection`/`TelloVideoDecoder`/`TelloVideoDisplay`. Check **Positioned Externally**, start inactive. |
...
If the app's build/rig uses **Floor Level** tracking origin (the common default on newer XR-Origin-based rigs) *and* Passthrough is enabled, Quest's OS treats it as a Roomscale experience requiring a properly configured Guardian boundary - if the headset doesn't have one set up (or has one in Stationary mode instead), the app can fail to launch entirely, with no error shown to the user (it just returns to the launcher). Confirmed via a device log showing `GuardianLaunchCheckHandler ... RequiresGuardian ... Launch failed`.

This project doesn't actually need Floor-calibrated tracking - `TelloUiKit.ComputeFixedPosition` already takes its own `assumedEyeHeightMeters` parameter rather than relying on the runtime's floor calibration. To match that and avoid the launch-blocking Guardian requirement:
- On the `Camera Offset` component: **Requested Tracking Mode → Device**
- On `OVR Manager`: **Tracking Origin Type → Eye Level** (not *Stationary* - that option is still experimental and has known accuracy issues with Unity's OpenXR-based components per Meta's own docs)

Both need to be changed together - they're meant to agree with each other, not independent settings.
...
- **Gamepad not detected if it pairs after the app has already launched** — still open. Confirmed via logs (across three separate sessions) that the OS-level Bluetooth HID connection succeeds, but the controller sometimes never shows up in Unity's Input System device list at all (not just `Gamepad.current` — `InputSystem.devices` itself stays empty). A fallback that scans the device list directly (rather than relying only on `.current`, which only updates after the pad sends its first input) helps in some cases but doesn't fully solve it. Likely needs a native Android-side bridge to catch the HID connect event directly. Turning the gamepad on *before* launching the app is the reliable workaround for now.
- **Wi-Fi auto-connect (West button) is built but currently disabled** — `TelloWifiConnector.java` + `TelloInitGate.ConnectToTelloWifi()` implement connecting to the Tello's hotspot by SSID prefix via Android's `WifiNetworkSpecifier` API, with a fallback to opening the system Wi-Fi settings panel if Wi-Fi itself is off. Turned out to be more trouble than it was worth in real testing, so the West button is currently a no-op (the legend card still shows "Connect Wi-Fi" so the layout doesn't need to change again if this comes back). The code is all still present and referenced by the project, just not wired to the input handler.
...
- East and West both trigger a photo in Piloting mode right now (a leftover from consolidating an older "menu mode" toggle - unrelated to the Menu screen's West button above). Decoupling Takeoff (South) from Land (East) is planned — one button always does one thing regardless of the drone's current state, which removes a small but real safety risk in the current toggle. Not yet implemented.
