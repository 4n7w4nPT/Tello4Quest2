...
{
    /// <summary>
    /// Thin, single-row banner ABOVE the video screen. Five small cards:
    /// Gamepad/Tello connection dots, the video signal meter, a last-command
    /// ok/fail dot, and temperature (color-coded against the warning/critical
    /// thresholds). Wider than the video screen on purpose - each card keeps
    /// the same physical size as before rather than shrinking to fit, same
    /// convention as TelloOptionsPanel below the screen. Parented under the
    /// head-locked video screen's transform, so it follows automatically.
    /// </summary>
    public class TelloStatusPanel : MonoBehaviour
    {
        ...
        // ReferenceCanvasPixelWidth is what the scale is anchored to - NOT
        // CanvasPixelWidth - so each card keeps the exact same physical (world)
        // size when a 5th card is added, and the bar correctly extends past the
        // video screen's edges instead of everything shrinking to fit.
        private const float CanvasPixelWidth = 1125f;
        ...
        private bool lastCommandOk;

        // =================================================================
        // RUNTIME-ADJUSTABLE SETTINGS (Settings screen)
        // =================================================================
        public float NominalFps { get => nominalFps; set => nominalFps = value; }
        ...
        private void Awake()
        {
            ...
            nominalFps = PlayerPrefs.GetFloat(PrefsPrefix + "NominalFps", nominalFps);
        ...
        {
            lastCommandKnown = true;
            lastCommandOk = success;
        }
        ...
        private void BuildUI()
        {
            ...
            lastCommandDot = BuildDotCard(canvasGO.transform, "Last cmd", new Vector2(ColumnX(3), 0f), cardSize);
            temperatureValue = BuildValueCard(canvasGO.transform, "Temp", new Vector2(ColumnX(4), 0f), cardSize);
        }
