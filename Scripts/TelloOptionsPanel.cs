...
{
    /// <summary>
    /// Thin, single-row banner BELOW the video screen, same width as it and the
    /// same rounded-card design as TelloStatusPanel above the screen. Five
    /// cards: speed level and sensitivity as simple "current/max" text
    /// indicators, plus altitude, flight time and ground speed as plain
    /// telemetry readouts. Parented under the video screen's transform, so it
    /// floats along with it.
    ///
    ...
    public class TelloOptionsPanel : MonoBehaviour
    {
        ...
        private void Update()
        {
            // Altitude/flight time/ground speed change continuously, unlike the
            // discrete speed/sensitivity levels (which only refresh on their
            // change events) - keep them live every frame.
